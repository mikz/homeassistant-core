...
class AuthToken:
    ...
    def __init__(self, access_token: str, refresh_token: str, expires_in: int, **rest) -> None:
    ...
    def valid_in(self, time: timedelta) -> bool:
        ...
        return self.access_token is not None and not self.expires(time or timedelta(seconds=0))
    
    def expires(self, time: timedelta) -> bool:
        """Return whether the token is expired."""
        _LOGGER.debug(f"Token expires at {self.expires_at} < {datetime.now() + time} ({time})")
...
class VeluxModule:
     ...
     def __eq__(self, other):
        if isinstance(other, VeluxModule):
            return self.home == other.home and self.id == other.id and self.type == other.type
        return False
...
class VeluxActiveAPI:
        ...
        async def authenticate(self, username: str, password: str) -> AuthToken:
            ...
            response = await self._websession.request("POST", OAUTH2_TOKEN, data={
                 "username": username, "password": password,
                 "grant_type": "password", "user_prefix": "velux",
        ...
        async def access_token(self) -> str:
            ...
            if not self.auth_token.valid_in(timedelta(hours=2, minutes=59)):
                try:
                    ...
                    _LOGGER.debug(f"Refreshed Auth Token. Now expires at {self.auth_token.expires_at} (in {self.auth_token.expires_in})")
        ...
        async def get_home_data(self) -> list[VeluxHome]:
            ...
            response = await self._websession.request("POST", API_URL + "/api/gethomedata", data={"access_token": access_token})
        ...
        async def get_home_statuses(self, home: VeluxHome) -> list[VeluxModule]:
            ...
            access_token = await self.access_token
            response = await self._websession.request("POST", API_URL + "/api/homestatus", data={"access_token": access_token, "home_id": home.id})
            ...
            return [VeluxModule(home, **module) for module in response_json['body']['home']['modules']]
...
class VeluxGatewayData:
    ...
    wifi_strength: int
    wifi_state: str
    outdated_weather_forecast: bool
